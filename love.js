...
const CONFETTI_COLORS = ['#f4c430', '#f0a93b', '#ffd966', '#c79a1f', '#fff2b8'];
...
const DEFAULT_HINT = 'Mantené presionado para tensar el arco — soltá para lanzar ♡';
const MISS_HINTS = ['¡Tan cerca! Probá de nuevo ♡', 'Apuntá al centro ♡', 'Nunca falla dos veces ♡'];
const ARROW_LEN = 56;
...
function sayYes() {
  catState = 'happy';
  title.textContent = '¡Sabía que te iba a gustar! ♡';
  drawSprite(catCanvas, CAT_HAPPY);
  catCanvas.setAttribute('aria-label', 'Un gatito feliz con una flor');
...
const LOVE_COLORS = [
  '#f4c430', '#ffd966', '#f0a93b', '#c79a1f', '#8bc34a',
  '#fff2b8', '#d9a404', '#fff8dc', '#a9c94a', '#f0c419', '#f7e7a1',
];
...
function reset() {
  catState = 'idle';
  title.textContent = '¡Sorpresa! 🌼 Flores amarillas para vos ¿Te gustó el detalle? ♡';
  drawSprite(catCanvas, CAT_IDLE);
  catCanvas.setAttribute('aria-label', 'Un gatito sosteniendo una flor');
...
const YT_VIDEO_ID = 'PykXcyfPUk4';

function onYouTubeIframeAPIReady() {
  ytPlayer = new YT.Player('ytPlayer', {
    videoId: YT_VIDEO_ID,
    playerVars: { autoplay: 1, mute: 1, loop: 1, playlist: YT_VIDEO_ID, controls: 1 },
    events: {
      onReady: () => { ytPlayer.setVolume(Number(volumeSlider.value)); },
    },
  });
}
...
function startMusic() {
  if (musicStarted || !ytPlayer || !ytPlayer.unMute) return;
  musicStarted = true;
  ytPlayer.unMute();
  isMuted = false;
  muteBtn.textContent = '🔊';
...
function toggleMute() {
  ...
  isMuted = !isMuted;
  if (isMuted) ytPlayer.mute(); else { ytPlayer.unMute(); musicStarted = true; }
...
if (volumeSlider){
  volumeSlider.addEventListener('input', () => {
    ...
    if (Number(volumeSlider.value) > 0 && isMuted){
      isMuted = false;
      ytPlayer.unMute();
      musicStarted = true;
      muteBtn.textContent = '🔊';
